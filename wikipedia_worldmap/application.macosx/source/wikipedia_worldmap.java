...
public class wikipedia_worldmap extends PApplet {
...
int mapcolorA, mapcolorB;
float rootMapOpacity = 1.0f;
...
public void keyPressed() {
  ...
  if (key=='p' || key=='P') {doLoop = true; savePDF = true; pointSize = 0.001f;}
...
public void message(boolean loading, String country) {
  if(loading) {
   message = "loading..."; 
  } else {
   message = country;
...
public void changeLanguageA(String language){
  message(true,"");
  String[] lines = loadStrings("data/csv/live_"+language+".csv");
  for (int i = 0; i < lines.length; i++) {  
    // do something with each line:
    String[] words = split(lines[i], ',');
    float x = map(PApplet.parseFloat(words[2]),-180,180,0,width);
    float y = map(PApplet.parseFloat(words[1]),90,-90,0,height);
...
public void changeLanguageB(String language){
  message(true,"");
  String[] lines = loadStrings("data/csv/live_"+language+".csv");
  for (int i = 0; i < lines.length; i++) {  
    // do something with each line:
...
public void controlEvent(ControlEvent theEvent) {
  ...
  if (theEvent.isGroup()) {
    ...
    if(theEvent.getGroup() == d1) {
      int theIndex = PApplet.parseInt(theEvent.getGroup().getValue());
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "wikipedia_worldmap" };
    if (passedArgs != null) {
